...
type HTTPModule struct {
	...
	handler       *warehouseHandler.HTTPHandler
	authenticator authService.Authenticator
}

func NewHTTPModule(cfg *config.Config, base *baseHandler.BaseHTTPHandler, handler *warehouseHandler.HTTPHandler, authenticator authService.Authenticator) appmodule.HTTPModule {
	return &HTTPModule{cfg: cfg, base: base, handler: handler, authenticator: authenticator}
}

func (m *HTTPModule) RegisterRoutes(r gin.IRouter) {
	v1 := r.Group("/api/v1")
	group := v1.Group("/warehouses")
