...
// customerBulkCreator is satisfied by the concrete customer service. Using a
// local interface lets the handler call BulkCreate without changing the shared
// CustomerService interface.
type customerBulkCreator interface {
	BulkCreate(ctx context.Context, items []models.CreateCustomerRequest) (*models.BulkImportResult, error)
}

// BulkCreate handles POST /api/v1/customers/bulk. It accepts a JSON array of
// customer payloads and imports them in one request, returning a per-row report.
func (h *HTTPHandler) BulkCreate(appCtx *app.Context) *app.CostumeResponse {
	var req models.BulkCreateCustomerRequest
	...
	creator, ok := h.service.(customerBulkCreator)
	if !ok {
		return &app.CostumeResponse{
			RequestID: appCtx.APIReqID,
